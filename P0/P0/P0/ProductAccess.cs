    ...
    class ProductAccess
    {
        public void getInventory()
        {
            ...
            while (dr.Read())
            {
                ...
                product.ProductDesc = dr[3].ToString();
                product.ProductSect = dr[4].ToString();
        ...
        public void getKiddieWorld()
        {
            string newStr = "Data source=JAMIESCHWARTZPC\\SQLEXPRESS;initial Catalog=P0;integrated security=true";
            SqlConnection prodSQL = new SqlConnection(newStr);
            string querystring = "SELECT ProductID, ProductName, ProductPrice, ProductDescription FROM dbo.FullProductList WHERE ProductSection = 'Kiddie World';";
            prodSQL.Open();
            SqlCommand cmd = new SqlCommand(querystring, prodSQL);
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
